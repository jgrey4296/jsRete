...
define(['./dataStructures'],function(DataStructures){
    ...
    var performJoinTests = function(joinNode,token,wme){
        ...
        for(var i in joinNode.tests){
            ...
            var wmeValue = wme.data[test.wmeField];
            //If the binding exists
            if(token.bindings[test.field2]){
                var tokenValue = token.bindings[test.boundVar];
    ...
    var joinNodeRightActivation = function(node,wme){
        ...
        if(node.alphaMemory.childNodes.length === 1){
    ...
    var negativeNodeLeftActivation = function(node,token,bindings,wme){
    ...
    var nccNodeLeftActivation = function(nccNode,token,bidings,wme){
    ...
    var nccParterNodeLeftActivation = function(partner,token,bindings,wme){
    ...
    var buildOrShareConstantTestNode = function(parent,testTuple){
        ...
        var newAlphaNode = new AlphaNode(parent,testTuple);
        return newAlphaNode;
    }
    ...
    var buildOrShareAlphaMemory = function(condition,root){
        ...
        var newAlphaMemory = new AlphaMemory(currentNode);
    ...
    var buildOrShareBetaMemoryNode = function(parent){
        ...
        var newBetaMemory = new BetaMemory(parent);
        //TODO:update it with matches
    ...
    var getJoinTestsFromCondition(condition){
    ...
    var buildOrShareJoinNode = function(parent,alphaMemory,tests){
        ...
        var newJoinNode = new JoinNode(parent,alphaMemory,tests);
        set the nearest ancestor
        newJoinNode.nearestAncestor = nearestAncestorWithSameAlphaMemory(parent,alphaMemory);
    ...
    var buildOrShareNegativeNode = function(parent,alphaMemory,tests){
        ...
        var newNegativeNode = new NegativeNode(parent,alphaMemory,tests);
    ...
    var buildOrShareNCCNodes = function(parent,conditions,rootAlpha){
        ...
        var newNCC = new NegatedConjunctiveConditionNode(parent);
        var newNCCPartner = new NegConjuConPartnerNode(bottomOfSubNetwork,conditions.length);
    ...
    var addRule = function(rule,reteNet){
        ...
        var actionNode = new ActionNode(currentNode,rule.action,rule.name);
        //update node with matches
        reteNet.actions[actionNode.name] = actionNode;
        return actionNode;
    }

    var leftActivate = function(node,token,joinTestResults,wme){
        if(node.isBetaMemory){
            betaMemoryLeftActivation(node,token,joinTestResults);
        }else if(node.isJoinNode){
            joinNodeLeftActivation(node,token);
        }else if(node.isNegativeNode){
            negativeNodeLeftActivation(node,token,bindings,wme);
        }else if(node.isAnNCCNode){
            nccNodeLeftActivation(node,token,bindings,wme);
        }else if(node.isAnNCCPartnerNode){
            nccPartnerNodeLeftActivation(node,token,bindings,wme);
